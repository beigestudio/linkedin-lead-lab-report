...
export const WelcomeStep = memo(({
  ...
  return <>
      ...
      <Dialog open={isPreparationModalOpen} onOpenChange={() => {}}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto touch-manipulation" onPointerDownOutside={e => e.preventDefault()}>
          <DialogHeader>
          ...
          <div className="space-y-6 px-2">
            <div className="grid grid-cols-1 gap-6">
              <PreparationItem number="1" title="Your LinkedIn Headline" description="The text that appears under your name on your profile" />
              <PreparationItem number="2" title="Your About Section" description="Your profile summary/bio (the main text in your About section)" />
            </div>
          ...
          <DialogFooter className="mt-6 sm:mt-8">
            <Button onClick={handleReadyToProceed} size="lg" className="w-full bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-primary-foreground px-6 sm:px-8 py-4 text-base sm:text-lg font-medium rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1 touch-manipulation min-h-[48px]">
        ...
        <CardContent className="pt-0 px-4 sm:px-6">
          <div className="bg-gradient-to-r from-primary/20 to-accent/20 rounded-2xl sm:rounded-3xl p-6 sm:p-8 text-foreground mb-8 sm:mb-10 backdrop-blur-sm border border-primary/20">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-8">
              <FeatureCard icon={CheckCircle} title="Expert Analysis" description="Personalized feedback from a strategist who's audited 500+ executive profiles" />
              <FeatureCard icon={TrendingUp} title="Actionable Roadmap" description="Clear steps to start generating qualified leads from your profile today" />
              <FeatureCard icon={Users} title="Personal Brand Focus" description="Strategies for executives to attract high-value prospects through personal branding" />
            </div>
          </div>

          <div className="text-center">
            <Button onClick={handleNext} size={isMobile ? "default" : "lg"} className="w-full sm:w-auto bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 text-primary-foreground px-8 sm:px-12 py-4 sm:py-6 text-base sm:text-lg font-medium rounded-xl sm:rounded-2xl shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1 touch-manipulation min-h-[48px]">
